...
export const generatePlanningGuides = async (idea: string, brandName?: string, apiKey?: string): Promise<Record<string, string[]>> => {
  ...
  const prompt = `
    ...
    **[작성 지침 - 쉽고 명확하게]**
    1. **현학적이고 어려운 단어 절대 금지.** (예: 본질적 결핍, 존재론적, 메타인지 등 금지)
    2. **직장인 주니어**가 봐도 바로 이해할 수 있는 **평이하고 쉬운 질문**을 만드세요.
    3. 질문은 구체적이어야 합니다. "가치가 무엇인가요?" 보다 "고객이 이 제품을 쓰면 기분이 어떻게 좋아지나요?"가 좋습니다.

    [질문 생성 구조 (3단계)]
    각 항목마다 다음 3단계 논리로 질문을 구성하세요:
    1. **질문 1 (문제 발견):** 고객이 지금 무엇 때문에 가장 불편해하거나 힘들어하나요?
    2. **질문 2 (해결책):** 우리는 그 문제를 남들과 다르게 어떻게 해결해주나요?
    3. **질문 3 (경험):** 그래서 고객은 결국 어떤 기분을 느끼게 되나요?
...
export const generateFieldDraft = async (
  ...
  const brandNameInstruction = isBrandNameField
    ? `
      **[중요: 브랜드 네이밍 미션]**
      ...
      사용자가 명확한 이름을 제시했다면 그것을 쓰고, 
      **없다면 당신이 답변 내용을 바탕으로 가장 잘 어울리는 이름을 지어서 제안하세요.**
  ...
  const prompt = `
    ...
    [요청]
    사용자의 3가지 답변을 하나로 모아서, **쉽고 명확한 전략 글**로 완성하세요.
    어려운 이론 용어는 빼고, **누구나 이해할 수 있는 상식적인 언어**로 작성해야 합니다.
    ...
    **작성 지침 (필수 준수):**
    ...
    3. **출력 구조 (반드시 지킬 것):**
       ...
       ### 우리의 생각
       이 문제를 대하는 우리의 태도나 믿음을 적습니다.

       ### 해결책
       우리가 어떻게 그 문제를 해결해주는지 구체적으로 적습니다.
       (반드시 브랜드명을 언급하세요.)

       ### 고객이 얻는 것
       그래서 고객의 삶이 어떻게 좋아지는지 적습니다.
