...
export const generatePlanningGuides = async (idea: string, brandName?: string, apiKey?: string): Promise<Record<string, string[]>> => {
  ...
  const prompt = `
    ...
    [당신의 역할: 브랜드 전략 멘토]
    당신은 사용자를 가르치는 선생님입니다. 단순히 정보를 묻는 것이 아니라, 
    사용자가 **스스로 깊이 생각하고** 진짜 통찰을 얻을 수 있도록 안내해야 합니다.

    [질문 작성의 철학적 기반 - 노골적으로 언급하지 말 것]
    
    **1. 고객 갈등 해결 중심 (Zero-Sum 사고 탈피)**
    - 잘못된 질문: "경쟁사를 이기려면 어떻게 해야 하나요?"
    - 올바른 질문: "고객이 현재 겪고 있는 진짜 문제는 무엇이고, 우리는 그것을 어떻게 새롭게 해결할 수 있나요?"
    - 핵심: 경쟁자를 이기는 것이 아니라, 고객 가치를 창조하는 것에 집중
    
    **2. 행동경제학적 통찰 반영**
    - 인간은 합리적이지 않습니다. 감정, 편향, 사회적 압력에 따라 움직입니다
    - 질문에 "고객이 진짜 원하는 것"과 "고객이 말하는 것"의 차이를 찾도록 유도하세요
    - 예: "고객이 '가격이 비싸다'고 말할 때, 그 뒤에 숨은 진짜 불안은 무엇일까요?"
    
    **3. 본질 탐구 (Why의 깊이)**
    - 표면적 답이 아닌, 3단계 깊이의 Why를 찾도록 유도
    - 예: "왜 이 사업을 하나요?" → "그 이유의 이유는?" → "그렇다면 진짜 변화시키고 싶은 것은?"
    
    **4. 차별화보다 독창성 (Differentiation vs Uniqueness)**
    - 차별화: 경쟁사 대비 조금 다른 것
    - 독창성: 아무도 하지 않았던, 오직 우리만의 것
    - 질문은 "남들과 달리"가 아니라 "우리만이 할 수 있는"에 초점

    [질문 생성 규칙]
    
    **길이와 구조:**
    - 각 질문은 **2-3문장**으로 구성
    - 첫 문장: 사고를 촉발하는 본질적 질문
    - 두 번째 문장: 구체적 예시나 사고 방향 제시
    - 세 번째 문장(선택): 추가 탐구 질문
    
    **톤과 태도:**
    - 질문하되 가르치는 선생님의 톤
    - "생각해보세요", "고민해보세요" 같은 직접적 지시어보다는
      자연스럽게 사고를 유도하는 질문 형태
    - 예: "만약 ~라면?" "~의 본질은 무엇일까요?" "~와 ~는 어떻게 다를까요?"
    
    **3가지 질문의 흐름 (Why → How → What 구조 유지하되 깊이 추가):**
    
    1. **질문 1 - 존재의 이유와 본질 (Why의 Why)**
       - 단순히 "왜?"가 아니라, 그 이유의 뿌리를 찾도록 유도
       - 고객 갈등의 본질, 우리 브랜드의 존재 이유
       - 예: "고객은 왜 현재 시장의 솔루션에 만족하지 못하나요? 그들이 느끼는 진짜 불편함, 좌절감의 뿌리는 무엇일까요?"
    
    2. **질문 2 - 방법론과 철학 (How의 깊이)**
       - 어떻게 다르게 접근할 것인지
       - 우리만의 독특한 방식, 철학, 태도
       - 예: "그 문제를 해결하는 우리만의 독특한 방식은 무엇인가요? 기존 방식과 근본적으로 다른 점이 있다면?"
    
    3. **질문 3 - 구체적 실행과 증거 (What의 명확성)**
       - 실제로 무엇을 보여줄 것인지
       - 측정 가능하고 관찰 가능한 결과
       - 예: "그 방식이 실제로 작동한다는 것을 고객에게 어떻게 증명하시겠어요? 구체적인 지표나 경험으로 보여줄 수 있나요?"

    [각 항목별 철학적 접근 힌트]
    
    **Identity (브랜드명, 철학, 슬로건)**
    - 이름과 철학은 브랜드의 영혼입니다
    - 유행을 따르기보다 시대를 관통하는 본질을 찾도록
    - "10년 후에도 의미있는가?"
    
    **Strategy (전략, 기술, 우위)**
    - 경쟁이 아닌 고객 가치 창조에 집중
    - "우리가 사라지면 고객은 무엇을 잃는가?"
    - RTB(Reason to Believe)의 구체성 요구
    
    **Customer (타겟, 문화, 가치)**
    - 인구통계 넘어 심리통계로
    - 고객의 숨겨진 욕망과 두려움 탐구
    - "그들이 말하는 것 vs 진짜 원하는 것"
    
    **Benefits (기능, 경험, 상징)**
    - 기능: 해결하는 Pain Point의 본질
    - 경험: 행동경제학적 비합리성 고려
    - 상징: 자아 이미지와 사회적 정체성
    ...
    **Management (키워드, 관계)**
    ...
    [절대 금지 사항]
    1. 이론 이름 언급 금지: "논백", "행동경제학", "골든서클" 등
    2. 학술 용어 금지: "존재론적", "본질적 결핍", "패러다임" 등
    3. 천편일률적 질문 금지: "무엇입니까?"를 3번 반복하지 말 것
    4. 짧은 질문 금지: 최소 2문장 이상의 생각을 유도하는 질문

    대상 항목: ${fieldsList}
    
    [출력 포맷]
    각 항목마다 3가지 질문을 생성하세요.
    각 질문은 2-3문장으로 구성하되, 사용자가 깊이 사고할 수 있도록 안내하는 멘토의 톤을 유지하세요.
    
    응답 포맷 (JSON):
    {
      "brandName": ["질문1 (2-3문장)", "질문2 (2-3문장)", "질문3 (2-3문장)"],
...
export const generateFieldDraft = async (
  ...
  const prompt = `
    ...
    **작성 지침 (필수 준수):**
    ...
    4. **쉬운 언어:** 현학적인 용어는 피하고, 진심이 느껴지는 명확한 언어를 사용하세요.
    5. **밀도 있는 분량:** 350자 내외.
