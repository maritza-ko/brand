...
export const generatePlanningGuides = async (idea: string, brandName?: string, apiKey?: string): Promise<Record<string, string[]>> => {
  ...
  const prompt = `
    ...
    **[작성 지침 - 천편일률적인 패턴 금지]**
    1. **항목별 맞춤형 질문:** 모든 항목에 똑같은 논리(문제-해결)를 적용하지 마세요. 각 항목의 본질을 꿰뚫는 질문을 던지세요.
       - 예: '브랜드 철학'이라면 신념과 약속을 묻고, '톤앤매너'라면 성격과 말투를 물으세요.
    2. **브랜드 페르소나 관점:** "우리가 누구에게 파는가?"(타겟)가 아니라 **"우리는 어떤 사람인가?"(페르소나)**를 정의하도록 유도하세요.
    3. **깊이 있는 사고 유도:** 사용자가 고민하게 만드는 통찰력 있는 질문을 던지세요. (단, 용어는 쉬워야 합니다.)

    [질문 구성]
    각 항목당 3개의 질문을 생성하세요.
    정해진 형식은 없습니다. 해당 항목을 가장 잘 정의할 수 있는 질문 3가지를 자유롭게 구성하세요.
...
export const generateFieldDraft = async (
  ...
  const brandNameInstruction = isBrandNameField
    ? `
      **[중요: 브랜드 네이밍 미션]**
      ...
      사용자의 답변을 바탕으로 **가장 잘 어울리는 브랜드명을 창작해서 제안하세요.**
  ...
  const prompt = `
    ...
    [요청]
    사용자의 답변을 바탕으로, 해당 항목에 대한 **진정성 있고 깊이 있는 전략 정의(Definition)**를 작성하세요.
    
    **작성 지침 (필수 준수):**
    1. **형식의 자유:** 천편일률적인 소제목(고객의 고민, 해결책 등)을 강제하지 않습니다. 
       이 항목의 성격을 가장 잘 드러낼 수 있는 구조로 자유롭게 작성하세요. 
       (필요하다면 소제목을 써도 좋고, 하나의 완결된 문단으로 써도 좋습니다.)
    2. **브랜드 페르소나 몰입:** 마치 브랜드가 살아있는 사람처럼, 그만의 성격과 철학이 묻어나게 작성하세요.
    3. **쉬운 언어:** 현학적인 용어는 피하고, 진심이 느껴지는 명확한 언어를 사용하세요.
    4. **밀도 있는 분량:** 350자 내외.
