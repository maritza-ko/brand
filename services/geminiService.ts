...
export const generatePlanningGuides = async (idea: string, brandName?: string, apiKey?: string): Promise<Record<string, string[]>> => {
  ...
  const prompt = `
    ...
    **[핵심 전략: 골든 서클 (Golden Circle) 적용]**
    질문이 단순히 "무엇입니까?"를 3번 반복하는 것은 최악입니다.
    사용자가 사고의 깊이를 더해갈 수 있도록 **Why -> How -> What**의 흐름으로 질문을 설계하세요.

    [질문 생성 구조 (3단계)]
    1. **질문 1 (Why - 본질과 신념):** 
       - 왜 이 항목이 필요한가요? 
       - 여기에 담고 싶은 가장 본질적인 가치나 신념은 무엇인가요?
       - (예: 브랜드명이라면 -> "이 이름을 통해 세상에 어떤 가치를 외치고 싶나요?")
    
    2. **질문 2 (How - 차별화와 방식):** 
       - 남들과 어떻게 다르게 표현할 것인가요?
       - 어떤 분위기, 톤, 혹은 독특한 방식으로 전달할 것인가요?
       - (예: 브랜드명이라면 -> "세련된 영어인가요, 친근한 한글인가요? 어떤 어감을 선호하나요?")

    3. **질문 3 (What - 구체적 결과):** 
       - 그래서 구체적으로 무엇을 보여줄 것인가요?
       - 떠오르는 키워드나 후보안, 혹은 고객이 보게 될 구체적 모습은?
       - (예: 브랜드명이라면 -> "머릿속에 있는 키워드나 후보 이름 3가지만 적어주세요.")

    **[주의사항]**
    - **절대 "이름이 무엇입니까?"라고 3번 묻지 마세요.**
    - 사용자가 생각의 단계를 밟아나갈 수 있도록 유도하세요.
    - 어려운 이론 용어(골든서클, Why, How 등)를 질문 텍스트에 직접 쓰지 말고, **질문의 논리에만 녹여내세요.**
